      ...
      <div className={formCard}>
        ...
        <form onSubmit={handleSubmit(onUserRegister)}>
          ...
          <div className={formGroup}>
            ...
            <input type="file" className={inputClass} accept="image/png, image/jpeg" {...register("profileImageUrl", {
              validate: {
                // Accepted file types:image/jpeg
                fileType: (files) => {
                  if (!files?.[0]) return true;
                  return ["image/png", image / jpeg].includes(files[0].type) || "Only JPEG allowed"
                },
                // size of the image is 2 MB here
                fileSize: (files) => {
                  if (!files?.[0]) return true;
              ...
              // To showing the image to the user before setting his profile picture for confirmation
              onChange={(event) => {
